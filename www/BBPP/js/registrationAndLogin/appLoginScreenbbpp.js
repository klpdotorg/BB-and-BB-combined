...
Game.appLoginScreenbbpp.prototype = {
	...
	startGame: function () {
		console.log("app login screen startgame");
		...
		if (_this.video == null) {
			_this.video = _this.add.video('demo');

		}

		_this.helpIcon = _this.add.image(500, 42, 'helpIcon');
		_this.helpIcon.scale.setTo(1);
		_this.helpIcon.anchor.setTo(0.5);
		// this.helpIcon.inputEnabled = true;
		// this.helpIcon.input.useHandCursor = true;
		_this.helpIcon.events.onInputDown.add(function () {
	...
	loadEverything: function (game) {
		...
		_this.fish = _this.game.add.sprite(150, 300, 'fish');
		_this.fish.scale.setTo(0.8);
		_this.fish.anchor.setTo(0.5);


		_this.butterfly = _this.game.add.sprite(390, 300, 'butterfly');
		_this.butterfly.scale.setTo(0.8);
		_this.butterfly.anchor.setTo(0.5);

		_this.flower = _this.game.add.sprite(150, 500, 'flower');
		_this.flower.scale.setTo(0.8);
		_this.flower.anchor.setTo(0.5);

		_this.parrot = _this.game.add.sprite(390, 500, 'parrot');
		_this.parrot.scale.setTo(0.8);
		_this.parrot.anchor.setTo(0.5);

		_this.sun = _this.Click_avatar_valuegame.add.sprite(150, 700, 'sun');
		_this.sun.scale.setTo(0.8);
		_this.sun.anchor.setTo(0.5);

		_this.tree = _this.game.add.sprite(390, 700, 'tree');
		...
		_this.fish.events.onInputDown.add(function () {
			for (var i = 0; i < _this.userArray.length; i++) {
				if (_this.userArray[i].name.toLowerCase() == "fish") {
					...
					//_this.state.start('index2bbpp',true,false,_this.user,false);//**If ONLINE 
		...
		var orline1 = _this.game.add.graphics(0, 0);
		...
		var orTxt = _this.game.add.text(game.world.centerX, 820, "or");
		...
		var orline2 = _this.game.add.graphics(0, 0);
		//var graphics=game.add.graphics(line.start.x,line.start.y);//if you have a static line
		...
		if (result.rows.length > 0) {
			...
			if (_this.userArray.length < 6) {
				...
				_this.regandstsrtBtn.events.onInputDown.add(function () {
					...
					//* see if it online you want you can call registrationLangSelectionScreenbbpp
					//* If it is offline you cannot call this call registrationPicSelectionScreenbbpp
	...
	localdatasuccess2: function (result) {
		...
		if (result.rows.length > 0) {
			...
			//* If it is offline you cannot call this call registrationPicSelectionScreenbbpp
			if (_this.app_Mode == "online") {
				console.log("fresh registration");
				_this.state.start('registrationLangSelectionScreenbbpp', true, false, _this.userArray, _this.app_Mode);
			} else {
