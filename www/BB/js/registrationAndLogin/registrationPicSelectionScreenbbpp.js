Game.registrationPicSelectionScreenbbpp = function () {

};

Game.registrationPicSelectionScreenbbpp.prototype = {
	...
	create: function (game) {
		...
		regBackArrow.events.onInputDown.add(function () {
			...
			if (_this.app_Mode == "online") {
				if (_this.user)
					game.state.start('registrationLangSelectionScreenbbpp', true, false, _this.user, _this.app_Mode);
				else
					game.state.start('registrationLangSelectionScreenbbpp', true, false, _this.app_Mode);
			} else {
				if (_this.user)
					game.state.start('appLoginScreenbbpp', true, false, _this.app_Mode, _this.language);
				else
					game.state.start('appLoginScreenbbpp', true, false, _this.app_Mode, _this.language);
			}
	...
	goback: function (e) {
		...
		if (_this.user)
			_this.state.start('registrationLangSelectionScreenbbpp', true, false, _this.user, _this.app_Mode);
		else
			_this.state.start('registrationLangSelectionScreenbbpp', true, false, _this.user, _this.app_Mode);
	},
